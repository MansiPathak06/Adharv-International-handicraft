...
const Furniture = () => {
  ...
  // Fetch Furniture products from backend
 // ✅ ENHANCED: Fetch Furniture products with DEBUG logging
// In Furniture.jsx, around line 28-43
useEffect(() => {
  async function fetchProducts() {
    ...
    try {
      ...
      // ✅ DEBUG: Check ALL categories in your database
      console.log('🔍 Total products in database:', data.products?.length);
      console.log('🔍 All unique categories:', [...new Set(data.products?.map(p => p.category))]);
      ...
      const furnitureProducts = (data.products || []).filter(p => {
        const category = (p.category || '').trim().toLowerCase();
        return category === 'furniture';
      });
      
      console.log('📦 Furniture products found:', furnitureProducts);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-red-50">
      ...
      <div
        ...
        style={{
          backgroundImage: 'url(https://i.pinimg.com/736x/40/71/13/407113ef50af05bf9a737eb4345c183f.jpg)',
        ...
        <div className="absolute inset-0 flex items-center">
          <div className="container mx-auto px-6 lg:px-16">
            <div className="max-w-2xl text-white space-y-4">
              <h1 className="text-5xl lg:text-6xl font-bold">Furniture</h1>
              <p className="text-xl text-amber-100">
                Statement Pieces That Define Your Living Space
              </p>
              <p className="text-lg text-amber-200">
                Crafted for Comfort, Strength, and Lasting Style
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="container mx-auto px-6 lg:px-16 py-12">
        ...
        <div className="text-sm text-gray-600 mb-8">
          Home / Products / <span className="text-amber-700 font-medium">Furniture</span>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="hidden lg:block w-80 flex-shrink-0">
            <div className="bg-white rounded-2xl shadow-lg p-6 sticky top-6">
              ...
              {/* Featured Banner */}
              <div className=" rounded-xl p-4 border-2 border-amber-300 bg-gradient-to-br from-amber-100 to-orange-100">
                <p className="text-sm font-semibold text-amber-900 mb-2">
                  ✨ Special Offer
                </p>
                <p className="text-xs text-amber-800">
                  Up to 50% off on selected furniture items
                </p>
              </div>
            </div>
          </aside>
                    ...
                    {(product.discount || (product.originalPrice && product.originalPrice > product.price)) && (
                      <div className="absolute top-4 left-4 bg-red-500 text-white px-3 py-1 rounded-full text-sm font-bold shadow-lg">
                        -{product.discount || Math.round(
                          100 * (1 - (product.price / (product.originalPrice || product.price)))
                        )}%
                      </div>
                    )}
                    <button
      ...
      <div className="bg-white py-12 mt-16">
        <div className="container mx-auto px-6 lg:px-16">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            {[
              { icon: '✓', title: 'Premium Quality', desc: 'Durable Furniture' },
