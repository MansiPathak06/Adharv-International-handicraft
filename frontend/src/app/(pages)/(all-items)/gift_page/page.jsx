...
const categories = [
  ...
  { id: 'figurines', name: 'Brass Figurines' },
  { id: 'wallDecor', name: 'Wall Decor' },
  { id: 'candleholders', name: 'Candle Holders' },
  { id: 'showpieces', name: 'Showpieces' },
  { id: 'accessories', name: 'Gift Accessories' }
];
...
const GiftPage = () => {
      ...
      try {
        ...
        setProducts(
          (data.products || []).filter(
            prod => (prod.category || '').toLowerCase() === 'gift items'
          )
        );
      } finally {
        setLoading(false);
  ...
  // Filtering by subcategory and price
  const filteredProducts = products.filter(product => {
    const catKey = (product.subcategory || '').toLowerCase();
    const categoryMatch =
  ...
  const sortedProducts = [...filteredProducts].sort((a, b) => {
    ...
    return (b.reviews || 0) - (a.reviews || 0); // "best-selling"
  });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-red-50">
      {/* Hero Banner */}
      <div 
        className={`relative h-80 overflow-hidden transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-10'}`}
        style={{
          backgroundImage: 'url(https://i.pinimg.com/1200x/79/0a/bd/790abd79e930470f293f1c66f2d693b5.jpg)',
        ...
        <div className="absolute inset-0 flex items-center">
          <div className="container mx-auto px-6 lg:px-16">
            <div className="max-w-2xl text-white space-y-4">
              <h1 className="text-5xl lg:text-6xl font-bold">Gift Items</h1>
              <p className="text-xl text-amber-100">
                Unwrap Handcrafted Gifts for All Occasions
              </p>
              <p className="text-lg text-amber-200">
                Thoughtful, Unique & Made with Love in India
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="container mx-auto px-6 lg:px-16 py-12">
        ...
        <div className="text-sm text-gray-600 mb-8">
          Home / Products / <span className="text-amber-700 font-medium">Gift Items</span>
        </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 sticky top-6">
              ...
              <div className="bg-gradient-to-br from-amber-100 to-orange-100 rounded-xl p-4 border-2 border-amber-300">
                ...
                <p className="text-xs text-amber-800">
                  Up to 50% off on premium gift items
                </p>
              </div>
            </div>
          </aside>
          ...
          <div className="flex-1">
                ...
                >
                  <div className="relative overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                      <button className="px-6 py-3 bg-white text-gray-800 rounded-lg font-semibold transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                        Quick View
                      </button>
                    </div>
                  </div>
            ...
            {!loading && sortedProducts.length === 0 && (
              <div className="text-center py-20">
                ...
                <p className="text-gray-600">
                  Try adjusting your filters to see more gift items
                </p>
              </div>
            )}
          </div>
      ...
      <div className="bg-white py-12 mt-16">
        <div className="container mx-auto px-6 lg:px-16">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            {[
              { icon: '✓', title: 'Handcrafted Gifts', desc: 'Unique and Memorable' },
              { icon: '🚚', title: 'Free Shipping', desc: 'On orders over ₹999' },
              { icon: '↺', title: 'Easy Returns', desc: '7 Days Return Policy' },
              { icon: '⭐', title: 'Loved Nationwide', desc: 'Trusted by Thousands' }
            ].map((badge, idx) => (
