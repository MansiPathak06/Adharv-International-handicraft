...
import { ChevronDown, Heart, ShoppingCart, Star, Filter, X } from 'lucide-react';
  ...
  useEffect(() => {
    async function fetchProducts() {
      ...
      try {
        ...
        // ✅ Find Table Tops products with multiple variations
        // ✅ FIXED: Case-insensitive category matching
const tableTopProducts = allProducts.filter((prod) => {
  const category = (prod.category || '').trim().toLowerCase(); // ✅ Convert to lowercase
  const variations = [
    'table tops',
    'tabletops', 
    'table top',
  ...
  if (match) {
    console.log('✅ Found Table Top product:', {
      id: prod.id,
      name: prod.name,
  ...
  return match;
        ...
        console.log(`✅ Total Table Tops found: ${tableTopProducts.length}`);
        
        setDebugInfo(
          `Found ${tableTopProducts.length} Table Tops products out of ${allProducts.length} total products`
        );
        
        setProducts(tableTopProducts);
      } catch (error) {
  ...
  // Filtering by price only (no subcategory)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-red-50">
      ...
      <div
        ...
        style={{
          backgroundImage:
            'url(https://i.pinimg.com/1200x/91/85/c8/9185c8bcaff6f6f2eb36de1a5dcf9863.jpg)',
        ...
        <div className="absolute inset-0 flex items-center">
          <div className="container mx-auto px-6 lg:px-16">
            <div className="max-w-2xl text-white space-y-4">
              <h1 className="text-5xl lg:text-6xl font-bold">Table Tops</h1>
              <p className="text-xl text-amber-100">
                Stylish Centerpieces for Tables and Consoles
              </p>
              <p className="text-lg text-amber-200">
                Thoughtfully Designed Accents for Everyday Elegance
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="container mx-auto px-6 lg:px-16 py-12">
        ...
        <div className="text-sm text-gray-600 mb-8">
          Home / Products /{' '}
          <span className="text-amber-700 font-medium">Table Tops</span>
        </div>
          ...
          <aside className="hidden lg:block w-80 flex-shrink-0">
            <div className="bg-white rounded-2xl shadow-lg p-6 sticky top-6">
              ...
              <div className="bg-gradient-to-br from-amber-100 to-orange-100 rounded-xl p-4 border-2 border-amber-300">
                <p className="text-sm font-semibold text-amber-900 mb-2">✨ Special Offer</p>
                <p className="text-xs text-amber-800">
                  Get up to 50% off on selected table tops
                </p>
              </div>
            </div>
          </aside>
            ...
            <div className="bg-white rounded-2xl shadow-md p-4 mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="px-4 py-2 text-gray-800 font-medium border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500"
                >
              ...
              {loading ? (
                ...
                <div className="col-span-full text-center py-20">
                  ...
                  <p className="text-gray-600 mb-4">
                    {products.length === 0 
                      ? 'No Table Tops products in database' 
                      : 'Try adjusting your price range'}
                  </p>
                  {/* ✅ Show raw product count */}
                  <p className="text-sm text-gray-500">
                    Raw products loaded: {products.length} | 
                    After filtering: {sortedProducts.length}
                  </p>
                </div>
              ) : (
                  ...
                  >
                    <div className="relative overflow-hidden">
                      ...
                      {(product.discount || (product.originalPrice && product.originalPrice > product.price)) && (
                        <div className="absolute top-4 left-4 bg-red-500 text-white px-3 py-1 rounded-full text-sm font-bold shadow-lg">
                          -{product.discount || Math.round(100 * (1 - (product.price / (product.originalPrice || product.price))))}%
                        </div>
                      )}
                      <button
                        onClick={(e) => {
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                        <button
                          ...
                          className="px-6 py-3 bg-white text-gray-800 rounded-lg font-semibold transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300"
                        >
                          View Details
                        </button>
                      </div>
                    ...
                    <div className="p-5">
                      ...
                      <button
                        ...
                        className="w-full py-3 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white font-semibold rounded-lg transition-all duration-300 flex items-center justify-center gap-2 group-hover:shadow-lg"
                      >
      ...
      <div className="bg-white py-12 mt-16">
        <div className="container mx-auto px-6 lg:px-16">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            {[
              { icon: '✓', title: 'Premium Quality', desc: 'Handcrafted Table Accents' },
              { icon: '🚚', title: 'Free Shipping', desc: 'On orders over ₹999' },
              { icon: '↺', title: 'Easy Returns', desc: '7 Days Return Policy' },
              { icon: '⭐', title: 'Trusted Brand', desc: 'Trusted by 50K+ customers' }
            ].map((badge, idx) => (
